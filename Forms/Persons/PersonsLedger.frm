...
Private Function FindRecordsAndPopulateGrid()
    ...
    If ValidateFields Then
        If RefreshList(txtInvoicePersonID.text, mskDateFrom(0).text, mskDateTo(0).text, txtDestinationDescription(0).text) > 0 Then
            UpdateRecordCount lblRecordCount, lngRowCount
            UpdateCriteriaLabels mskDateFrom(0).text, mskDateTo(0).text, txtPersonDescription.text, txtDestinationDescription(0).text
            If txtInvoiceMasterRefersTo.text = "1" Then
    ...
    With grdPersonsIndex
        For lngRow = 1 To .RowCount
            If .CellIcon(lngRow, "Selected") >= 1 Then
                If RefreshList(.CellValue(lngRow, "ID"), mskDateFrom(1).text, mskDateTo(1).text, txtDestinationDescription(1).text) > 0 Then
                    If printOrCreatePDF = "Print" Then DoReport "Print", txtCustomersOrSuppliers.text, .CellValue(lngRow, "Description"), mskDateFrom(1).text, mskDateTo(1).text
                    If printOrCreatePDF = "CreatePDF" Then DoReport "CreatePDF", txtCustomersOrSuppliers.text, .CellValue(lngRow, "Description"), mskDateFrom(1).text, mskDateTo(1).text
                End If
            End If
        Next lngRow
    End With
    ...
    Select Case Index
            ...
            If txtBatchReport.text = "Yes" Then
                If ValidateFields Then
                    If PopulatePersonsIndex > 0 Then
                        UpdateRecordCount lblRecordCount, lngRowCount
                        UpdateCriteriaLabels mskDateFrom(1).text, mskDateTo(1).text, txtPersonDescription.text, txtDestinationDescription(1).text
        ...
        Case 2
            ...
            If txtBatchReport.text = "No" Then
                ...
                DoReport "Print", txtCustomersOrSuppliers.text, txtPersonDescription.text, mskDateFrom(0).text, mskDateTo(0).text
            End If
            'Πολλές καρτέλες
            If txtBatchReport.text = "Yes" Then
        ...
        Case 3
            'Μία καρτέλα
            If txtBatchReport.text = "No" Then
                If DoReport("CreatePDF", txtCustomersOrSuppliers.text, txtPersonDescription.text, mskDateFrom(0).text, mskDateTo(0).text) Then
                    MyMsgBox 1, strApplicationName, strStandardMessages(8), 1
                End If
            End If
            'Πολλές καρτέλες
...
Private Function DoReport(action As String, Persons As String, PersonDescription As String, fromDate As String, toDate As String)
    ...
    If action = "CreatePDF" Then
        If Persons = "Customers" Then
            If CreateUnicodeFileForCustomers("ΚΑΡΤΕΛΑ ΠΕΛΑΤΗ " & SanitizeString(PersonDescription), " από " & fromDate & " έως " & toDate, GetSetting(strApplicationName, "Settings", "Export Report Height")) Then
                If CreateUnisexPDF("ΚΑΡΤΕΛΑ ΠΕΛΑΤΗ " & " " & SanitizeString(PersonDescription), rptOneLiner, 7) Then
                    DoReport = True
                End If
            End If
        End If
        If Persons = "Suppliers" Then
            If CreateUnicodeFileForSuppliers("ΚΑΡΤΕΛΑ ΠΡΟΜΗΘΕΥΤΗ " & SanitizeString(txtPersonDescription.text), " από " & fromDate & " έως " & toDate, GetSetting(strApplicationName, "Settings", "Export Report Height") - 4) Then
                If CreateUnisexPDF("ΚΑΡΤΕΛΑ ΠΡΟΜΗΘΕΥΤΗ " & SanitizeString(PersonDescription), rptOneLiner, 7) Then
                    DoReport = True
                End If
            End If
        End If
...
Private Function ValidateFields()
    ...
    'Από
    If mskDateFrom(0).text = "" And txtBatchReport = "No" Then
        If MyMsgBox(4, strApplicationName, strStandardMessages(1), 1) Then
        End If
        mskDateFrom(0).SetFocus
        Exit Function
    End If
    
    'Εως
    If mskDateTo(0).text = "" And txtBatchReport = "No" Then
        If MyMsgBox(4, strApplicationName, strStandardMessages(1), 1) Then
        End If
        mskDateTo(0).SetFocus
        Exit Function
    End If
    
    'Σωστό διάστημα
    If IsDate(mskDateFrom(0).text) And IsDate(mskDateTo(0).text) And txtBatchReport = "No" Then
        If CDate(mskDateFrom(0).text) > CDate(mskDateTo(0).text) Then
...
Private Sub Form_Activate()
    ...
    If Me.Tag = "True" Then
        ...
        AddColumnsToGrid grdCustomersLedger, False, 44, GetSetting(strApplicationName, "Layout Strings", "grdCustomersLedger"), _
            ...
            "Ημερομηνία,Παραστατικό,Προορισμός,Ενήλικες,Παιδιά,Δωρεάν,Χρέωση ενηλίκων,Χρέωση παιδιών,Απευθείας ποσό,Σύνολο χρέωσης,Πίστωση,Υπόλοιπο,A,B,E,TrnID"
        AddColumnsToGrid grdSuppliersLedger, False, 44, GetSetting(strApplicationName, "Layout Strings", "grdSuppliersLedger"), _
            ...
            "Ημερομηνία,Παραστατικό,Κατηγορία εξόδου,Χρέωση,Πίστωση,Υπόλοιπο,A,B,E,TrnID"
...
Private Sub Form_Load()
    ...
    ClearFields mskDateFrom(0), mskDateTo(0), mskDateFrom(1), mskDateTo(1), txtPersonDescription, txtDestinationDescription(0), txtDestinationDescription(1)
