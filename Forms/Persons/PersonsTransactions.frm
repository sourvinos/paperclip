...
Private Function AbortProcedure(blnStatus)
    ...
    If Not blnStatus Then
        If MyMsgBox(3, strApplicationName, strStandardMessages(3), 2) Then
            ...
            ClearFields mskDate, mskDateRefersTo, txtCodeShortDescriptionA, txtInvoiceNo, txtPersonDescription, txtReason, txtPaymentWayDescription, txtBankDescription, txtCheckNo, mskCheckExpireDate, mskAmount
            DisableFields mskDate, mskDateRefersTo, txtCodeShortDescriptionA, txtInvoiceNo, txtPersonDescription, txtReason, txtPaymentWayDescription, txtBankDescription, txtCheckNo, mskCheckExpireDate, mskAmount
...
Private Function NewRecord()
    ...
    ClearFields txtInvoiceID, txtInvoiceTrnID, txtInvoiceCodeID, txtInvoicePersonID, txtInvoiceDateIn, txtPaymentPaymentWayID, txtPaymentBankID, txtCodeLastNo, txtCodeLastDate, txtCodePersonsPlusOrMinus, chkCodeHandID
    ClearFields lblCodeDescription, lblCodeBatch, lblCodeHand
    ClearFields mskDate, mskDateRefersTo, txtCodeShortDescriptionA, txtInvoiceNo, txtPersonDescription, txtReason, txtPaymentWayDescription, txtBankDescription, mskAmount
    EnableFields mskDate, mskDateRefersTo, txtCodeShortDescriptionA, txtInvoiceNo, txtPersonDescription, txtReason, txtPaymentWayDescription, txtBankDescription, txtCheckNo, mskCheckExpireDate, mskAmount
    ...
    InitializeFields mskDate, mskAmount, txtPaymentBankID
...
Private Function AskToPrintReceipt()
    ...
    If chkCodeHandID.Value = 0 Then
        If MyMsgBox(2, strApplicationName, strAppMessages(7), 2) Then
            ProcessSelectedReceiptsForPrinting txtInvoiceTrnID.text, False, arrDummy
        End If
    End If
...
Private Function ValidateFields()
    ...
    If CDate(mskDate.text) > Date Then
    ...
    If blnStatus Then
        If chkCodeHandID.Value = 0 Then
            If CheckForDuplicateInvoice(mskDate.text, txtInvoiceCodeID.text, txtInvoiceNo.text) Then
    ...
    If blnStatus Then
        If chkCodeHandID.Value = 0 Then
            If CDate(mskDate.text) < CDate(txtCodeLastDate) Then
...
Private Sub cmdButton_Click(Index As Integer)
    ...
    Select Case Index
        ...
        Case 2
            ProcessSelectedReceiptsForPrinting txtInvoiceTrnID.text, False, arrDummy
        Case 3
            ProcessSelectedReceiptsForPrinting txtInvoiceTrnID.text, True, arrDummy
        Case 4
            DeleteRecord
        Case 5
            FindRecords
...
Public Function ProcessSelectedReceiptsForPrinting(strInvoiceTrnID, canCreatePDF As Boolean, arrInvoicesTrnID())

    Dim intLoop As Integer
    Dim rstRecordset As Recordset
    
    If Not canCreatePDF Then If Not SelectPrinter("PrinterPrintsReports") Then Exit Function
    If Not canCreatePDF Then If Not PrinterExists(strPrinterName) Then Exit Function
    ...
    For intLoop = 0 To UBound(arrInvoicesTrnID)
        ...
        PrintThisReceipt blnPreviewInvoices, canCreatePDF, rstRecordset!InvoiceNo
    Next intLoop
    
    If canCreatePDF Then MyMsgBox 1, strApplicationName, strStandardMessages(8), 1
...
Private Function CheckFunctionKeys(KeyCode, Shift)
    ...
    Select Case KeyCode
        ...
        Case vbKeyP And CtrlDown And cmdButton(2).Enabled
            cmdButton_Click 2
        Case vbKeyF3 And cmdButton(4).Enabled, vbKeyD And CtrlDown And cmdButton(4).Enabled
            cmdButton_Click 4
        Case vbKeyF7 And cmdButton(5).Enabled, vbKeyF And CtrlDown And cmdButton(5).Enabled
            cmdButton_Click 5
        Case vbKeyEscape
...
Private Sub mskDateRefersTo_GotFocus()

    If mskDate.text = "" Then
        ClearFields mskDateRefersTo
    End If

    If mskDateRefersTo.text = "" Then
        mskDateRefersTo.text = mskDate.text
    End If
...
Public Function DoPostFoundJobs(rstRecordset As Recordset, strPaymentInOrPaymentOut, strCustomersOrSuppliers)
    ...
    UpdateButtons Me, 7, _
        0, _
        1, _
        IIf(chkCodeHandID.Value = 1, 0, 1), _
        IIf(chkCodeHandID.Value = 1, 0, 1), _
        IIf(CheckForTheLastInvoice = 0, 0, 1), _
        IIf(CheckForLoadedForm("InvoicesOutIndex"), 0, 1), _
        IIf(chkCodeHandID.Value = 1, 1, 0), _
        IIf(chkCodeHandID.Value = 1, 0, 1)
...
Private Function PopulateFields(rstRecordset As Recordset)
    ...
    With rstRecordset
        ...
        mskDate.text = format(!InvoiceDateIssue, "dd/mm/yyyy")
